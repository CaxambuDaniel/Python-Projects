...
# Gera a lista de clientes a partir do diretório de clientes
...
# Checa a quantidade de diretórios para um cliente específico
def checa_qtd_diretorios(cliente, logger):
    ...
    # Se a quantidade de diretórios for maior que 2, gera a lista de quantidade de diretórios e deleta o diretório mínimo
    if qtd_dir > 2:
...
# Compacta os arquivos de carga para a lista de clientes
def compacta_arquivos_carga(lista_clientes, logger):
    ...
    for cliente in lista_clientes:
        ...
        # Se o diretório de ontem existir, compacta os arquivos de ontem e hoje
        ...
        # Caso contrário, compacta apenas os arquivos de hoje
        else:
            logger.debug(f"Compactando arquivos no diretório de hoje: {dir_hoje}")
            os.system(f"find {dir_hoje} -type f ! -iname '*.gz' -exec gzip -f {{}} \\;")
...
# Deleta o diretório mínimo para um cliente específico
def deleta_dir_min(cliente, lista_qtd_dir, logger):
    logger.debug(f"Deletando o diretório mínimo para o cliente: {cliente}")

    # Se a lista de quantidade de diretórios não estiver vazia, deleta o diretório mínimo
    if lista_qtd_dir:
